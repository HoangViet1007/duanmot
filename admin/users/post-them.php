<?php 
   ...
   if($idErr.$mat_khauErr.$cf_mat_khauErr.$ho_tenErr.$hinhErr.$emailErr.$so_chung_minhErr.$sdtErr){
       header("location:".BASE_URL."admin/users/them.php?iderr=$idErr&mat_khauerr=$mat_khauErr&cf_mat_khauerr=$cf_mat_khauErr&ho_tenerr=$ho_tenErr&emailerr=$emailErr&so_chung_minherr=$so_chung_minhErr&sdterr=$sdtErr&hinherr=$hinhErr") ;
       die() ;  
   }
    //    var_dump(1) ; die() ; 
   ...
   $cout->execute();
   if ($cout->rowCount() > 0) {
       header("location:".BASE_URL."admin/users/list.php?msg=Users này đã tồn tại !") ; 
       die();  
   } else {
       try {
           $sql = "INSERT INTO users(id,mat_khau,ho_ten,hinh,email,so_chung_minh,sdt,ma_vai_tro)
           VALUES 
                                   ('$id','$mat_khau','$ho_ten','$path','$email','$so_chung_minh','$sdt',$ma_vai_tro)" ;       
       
           insert_update_delete($sql) ; 
           header("location:".BASE_URL."admin/users/list.php?msg=Thêm users thành công !") ; 
