<?php
    ...
    $data = 6;
    $sql = "SELECT count(*) FROM `hinh_anh_khach_san`";
    $conn = connect();
    ...
    $sql = "select * from hinh_anh_khach_san LIMIT $tin , $data" ; 
    $tt = select_all($sql) ; 
...
<div class="page-header" style=" background-color: #FAFAFA;box-shadow: 0px 5px 20px 0px rgba(0, 0, 0, 0.1);">
    <div class="container-fluid">
        <h2 class="h5 no-margin-bottom">Danh sách hình ảnh thư viện</h2>
    </div>
</div>
...
<div class="conten"
        ...
        <div class="col-md-12">
            <table class="table table-hover" style="font-size: 13px; border: 1px solid #dedede;">
                <thead style="background-color: #2D3035;">
                    ...
                    <th width="110">
                        <a href="<?= BASE_URL?>admin/thu_vien/them.php" class="btn btn-info"
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <img src="<?php echo BASE_URL. $key['hinh'] ?>" class="img-fluid" width="60" height="60"
                                alt="">
                        </td>
                        <td><?php echo $key['duong_dan']?></td>
                        <td>
                            <a href="<?php echo BASE_URL?>admin/thu_vien/sua.php?id=<?= $key['id'] ?>"
                                class="btn btn-info btn-sm">
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                            </a>

                            <a onclick="return confirm('Bạn có chắc chắn muốn xóa loại hàng này không ?')"
                                href="<?php echo BASE_URL?>admin/thu_vien/xoa.php?id=<?= $key['id'] ?>"
    ...
    <?php
                                        for ($t = 1; $t <= $page; $t++) { ?>
            &nbsp;<a name="" id="" class="btn btn-secondary" href="<?php echo BASE_URL ?>admin/thu_vien/list.php?product=<?= $t ?>" role="button">
                <?= $t ?></a>
            <?php
                }
                ?>
